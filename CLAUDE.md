...
**Homepage category badges + curved section divider** (`index.astro`, "Explore the Menu" → "Our Signature"): Two techniques working together, both no-dependency CSS/inline-SVG:
- *Badge overlap*: `.category-grid` (the four circular icon tiles) is nudged down with `transform: translateY(...)` — a paint-only shift that doesn't affect document flow/height, avoiding margin-collapse issues. `.categories` gets `position: relative; z-index: 2` so it paints above the static `.featured` section below it, letting the badges visually spill into the surface below. `.featured`'s top padding is increased to keep clearance before the burger content.
- *Curved top edge*: `.featured-wave` is an inline SVG (viewBox `0 0 1600 60`, `preserveAspectRatio="none"`) holding one closed path filled `var(--color-bg)`, absolutely positioned at the top of `.featured`. It's the same "hand-drawn swoosh" motif as the wordmark underline (`Nav.astro`'s `.nav-logo-text::after`), but **not** the same path data — literally reusing the logo's tiny 100-unit-wide curve stretched non-uniformly to a ~1600px-wide section distorted its two close-together bezier control points into an odd double-bump. The divider path was redrawn with control points proportioned for full-bleed width, keeping the same single-sweep, asymmetric character.
- *Why the badge offset looks bigger than you'd expect*: the curve rises close to flat almost exactly under the badge cluster's x-position, which eats into the effective overlap. The `transform: translateY()` values (`6rem` desktop, `7rem` in the `max-width: 768px` block) were tuned by sampling actual rendered pixel colors under the badges/curve, not by measuring against a flat section boundary — the flat-boundary math undersells how much push is needed by roughly 40-50px. If either the curve path or badge size changes, re-tune by measuring against the rendered curve, not the section's plain bounding box.
- *Extending this further*: the divider is only on the top of `.featured` today. To use it elsewhere — e.g. as a bottom edge closing a section into the one below, or mirrored/flipped (`transform: scaleY(-1)` on the SVG, or a re-authored path) for a different visual rhythm — treat each placement as its own tuning pass: redraw/scale the path for that section's actual pixel width, and if anything is meant to overlap it (like the badges here), verify the overlap with a pixel probe against the real curve rather than the section's flat top/bottom.
